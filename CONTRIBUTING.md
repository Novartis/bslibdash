...
- The bslibdash version (`packageVersion("bslibdash")`) and the output
  of [`sessionInfo()`](https://rdrr.io/r/utils/sessionInfo.html) or
...
discuss the approach. That way we can make sure it is a good fit for the
package and avoid duplicated effort.
...
6.  Push the branch and open a pull request. Reference the related issue
    in the description.
...
- Bug fixes should include a regression test that fails before your fix
  and passes after it.
- Snapshot tests
  ([`testthat::expect_snapshot()`](https://testthat.r-lib.org/reference/expect_snapshot.html))
  are welcome for HTML output; commit the resulting `_snaps/` files.
